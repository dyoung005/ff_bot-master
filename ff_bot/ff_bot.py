...
def random_phrase():
    phrases = ['Billy Mays here with another fantastic product!', 'Before you throw it away, let Mighty Mendit save the day!',
               'Now that\'s the power of Mighty Putty!', 'Don\'t just get it clean, get it OxiClean!',
               'Topped with pickles, onions, ketchup or cheese, Big City Sliders are sure to please!', 'What would happen if you combine a cheese greater with a plate?',
               'It\'s faster than a knife and the only tears you\'ll cry are tears of joy!',
               'Kaboom! And the soap scum is gone!', 'Even after continuous pounding these M&M\'s don\'t break!, 'But whether it\'s scissors, tools or knives, big or small, the Samurai Shark shapens them all!',
               'But through this exclusive TV Offer, we'll send you a second free!', 'Order now for only $19.99!', 'Zobreez attracts liquid like a magnet!']
...
def get_power_rankings(league):
    ...
    score = ['%s - %s' % (i[0], i[1].team_name) for i in pranks
             if i]
    text = ['This Week\'s Power Rankings'] + score + random_phrase()
    return '\n'.join(text)
